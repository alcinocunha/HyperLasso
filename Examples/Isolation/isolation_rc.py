...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print("Usage: python isolation_rc.py <transactions> <keys> <values>")
        sys.exit(1)
    ...
    def install(t):
        ...
        stmts.append("(" + " & ".join([f"(reads_{t}_{k} = 0 | (" + " | ".join([f"reads_{t}_{k} = writes_{t2}_{k} & installed_{t2}" for t2 in range(T) if t2 != t]) + "))" for k in range(K)]) + ")")
