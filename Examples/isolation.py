...
if __name__ == "__main__":
    ...
    print("    serializable : boolean;") # otherwise we model READ COMMITTED
    ...
    def install(t):
        ...
        stmts.append("(serializable -> (" + " & ".join([f"(reads_{t}_{k} = 0 | reads_{t}_{k} = value_{k})" for k in range(K)]) + "))")
        stmts.append("(!serializable -> (" + " & ".join([f"(reads_{t}_{k} = 0 | (" + " | ".join([f"reads_{t}_{k} = writes_{t2}_{k} & installed_{t2}" for t2 in range(T) if t2 != t]) + "))" for k in range(K)]) + "))")
    ...
    print("    forall A . exists B . !(!serializable[A] & " + all_installed("A") + " & ( serializable[B] & " + same_transactions("A","B") + " -> !(" + all_installed("B") + ")))" )